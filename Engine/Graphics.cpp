...
{
	Vector2d<int> coord(0, 0);
	return RectI(coord, ScreenWidth, ScreenHeight);
}
...
void Graphics::DrawSprite(int x, int y, RectI rect, const RectI& clipRect, const Surface& s, const Color& chroma)
{
	...
	for (int sy = rect.top; sy < rect.bottom; sy++) {
		for (int sx = rect.left; sx < rect.right; sx++) {
			...
			if (pixel != chroma) {
				PutPixel(x + sx - rect.left, y + sy - rect.top, pixel);
			}
		}
	}
}
...
void Graphics::DrawSpriteNonChroma(int x1, int y1, const Surface& s)
{
	...
	DrawSpriteNonChroma(x1, y1, s.GetRect(), s);
}
...
void Graphics::DrawSpriteNonChroma(int x, int y, RectI rect, const RectI& clipRect, const Surface& s)
{
